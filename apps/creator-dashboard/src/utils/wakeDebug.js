...
function logApiCall(method, path, duration, status, responseSize, error) {
  ...
  methods.forEach(method => {
    ...
    apiClient[method] = async function(...args) {
      ...
      try {
        ...
        logApiCall(method.toUpperCase(), path, duration, 200, size, null);
        return result;
      } catch (err) {
        const duration = performance.now() - start;
        logApiCall(method.toUpperCase(), path, duration, err.status || 0, null, err.message);
        throw err;
      }
    };
  });
