...
export default function MediaPickerModal({ isOpen, onClose, onSelect, accept = 'image/*' }) {
  ...
  // Filter by search only — non-matching accept types are shown but disabled
  ...
  const handleFiles = useCallback((fileList) => {
    ...
    enqueue(files, (completedItem) => {
      setItems((prev) => {
        if (prev.some((p) => p.id === completedItem.id)) return prev;
  ...
  const uploadingForThisModal = activeItems;
  const hasQueueActivity = uploadingForThisModal.length > 0;
  ...
  return (
    <div className="mp-backdrop" ref={backdropRef} onClick={handleBackdropClick}>
      <div className="mp-container" role="dialog" aria-modal="true" aria-label="Carpeta de medios">
        ...
        <div className="mp-header">
          <div className="mp-header-left">
            <h2 className="mp-title">
              {isVideoPicker ? 'Tus vídeos' : 'Tu carpeta de medios'}
            </h2>
            {items.length > 0 && (
        ...
        {/* Queue strip */}
        {hasQueueActivity && (
          <div className="mp-queue-strip">
            <div className="mp-queue-strip-inner">
              {uploadingForThisModal.map((q) => (
                <div key={q.queueId} className="mp-queue-chip">
                  <div className="mp-queue-chip-thumb">
                    {q.thumbnailUrl ? (
                      <img src={q.thumbnailUrl} alt="" />
                    ) : (
                      <div className="mp-queue-chip-placeholder" />
                    )}
                    <svg className="mp-queue-chip-ring" viewBox="0 0 36 36">
                      ...
                      <circle
                        ...
                        strokeDashoffset={100.5 * (1 - (q.progress || 0) / 100)}
                        transform="rotate(-90 18 18)"
                      />
                    </svg>
                  </div>
                  <div className="mp-queue-chip-info">
                    <span className="mp-queue-chip-name">{q.file.name}</span>
                    <span className="mp-queue-chip-status">
                      {q.status === STATUS.COMPRESSING && 'Optimizando...'}
                      {q.status === STATUS.UPLOADING && `${q.progress}%`}
                      {q.status === STATUS.QUEUED && 'En cola'}
                    </span>
                  </div>
                </div>
              ))}
        ...
        >
          ...
          {/* Empty state */}
          {!loading && !error && filteredItems.length === 0 && !hasQueueActivity && (
            <div className="mp-empty">
              ...
              <p className="mp-empty-title">Tu carpeta está vacía</p>
        ...
        {items.length > 0 && (
          <div className="mp-footer">
            <span className="mp-footer-hint">
              Arrastra archivos o usa <kbd>Subir</kbd> • Las imágenes se optimizan automáticamente
            </span>
          </div>
        )}
      </div>
