...
import {
  ...
  Pencil, Trash2, X, Check, RotateCcw, Circle,
} from 'lucide-react';
...
export default function LoomRecorder({ videoSrc, onComplete, onCancel }) {
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      if (err && err.name === 'NotAllowedError') {
        ...
        setSetupError('Tu navegador no permite compartir esta pestaña. Usa Chrome o Edge para mejor compatibilidad.');
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    let raf;
    const tick = () => {
      const hasPointer = strokesRef.current.some((s) => s.mode === 'pointer' && s.endedAt);
      if (hasPointer) redraw();
  ...
  }, [phase, handleCancelAll]);

  const toggleMicLive = useCallback((next) => {
    setMicOn(next);
    if (userStream) {
      userStream.getAudioTracks().forEach((t) => { t.enabled = next; });
    }
  }, [userStream]);
      ...
      <div className="lr-root lr-root--setup">
        ...
        <div className="lr-setup-card">
          ...
          <p className="lr-setup-hint">
            Tu navegador pedirá permiso para compartir esta pestaña.
            Selecciona <strong>"Esta pestaña"</strong> con la opción <strong>"Compartir audio"</strong> activada.
          </p>
        </div>
      </div>
    );
      ...
      <div className="lr-root">
        ...
        {phase === 'recording' && (
          <RecordingBar
            ...
            micOn={micOn}
            onToggleMic={() => toggleMicLive(!micOn)}
            camOn={camOn}
...
function RecordingBar({
  ...
  drawTool, setDrawTool, drawColor, setDrawColor, drawWidth, setDrawWidth, drawMode, setDrawMode,
  onClearAll,
  micOn, onToggleMic,
  camOn, bubbleVisible, onToggleBubble,
}) {
  const penOpen = drawTool === 'pen';
  return (
    <div className="lr-bar">
      ...
      <div className="lr-bar-tool">
        <button
          className={`lr-bar-btn ${penOpen ? 'lr-bar-btn--active' : ''}`}
          onClick={() => setDrawTool(penOpen ? null : 'pen')}
          title="Dibujar"
        >
          <Pencil size={14} />
        </button>
