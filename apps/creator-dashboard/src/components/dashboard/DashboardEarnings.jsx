...
import { TrendingUp, Repeat, Users, ShoppingBag, Receipt } from 'lucide-react';
...
function MoneyTile({ label, sublabel, totals, currencies, field }) {
  ...
  return (
    <Card>
      ...
      <div className="earn-count">
        <span className="earn-count__icon">{icon}</span>
...
function EarnTooltip({ active, payload, label }) {
  if (!active || !payload?.length) return null;
  return (
    <div className="earn-tooltip">
      <span className="earn-tooltip__label">{fmtMonth(label)}</span>
      {payload.map((e) => (
  ...
  return (
    <Card className="earn-program">
      ...
      <div className="earn-money-list">
        {curs.map((c) => (
          <div key={c} className="earn-money-row">
            <span className="earn-money-value earn-money-value--sm">
              {fmtMoney(program.currencies[c]?.revenue ?? 0, c)}
            </span>
            <span className="earn-cur-tag" style={{ color: CUR_COLOR[c] }}>{c}</span>
          </div>
        ))}
...
function DashboardEarnings({ programId = null }) {
  ...
  if (isLoading) {
    return (
      <div className="earn-wrap">
        <div className="earn-kpis">{[0, 1, 2, 3].map((i) => <SkeletonCard key={i} />)}</div>
      </div>
    );
  }
  if (isError) {
    return (
      <div className="earn-wrap">
        <InlineError message="No se pudo cargar los ingresos." field="earnings" />
      </div>
    );
  }
  if (!scoped) return null;

  const { totals, currencies, programs, byMonth, orders, commissionRate } = scoped;
  ...
  const chartCurrencies = orderCurrencies(
    byMonth.reduce((set, m) => {
      Object.keys(m).forEach((k) => { if (k !== 'month' && !set.includes(k)) set.push(k); });
      return set;
    }, [])
  );
  const commissionPct = Math.round((commissionRate ?? 0) * 100);
    ...
    <div className="earn-wrap">
      {!programId && (
        <div className="earn-header">
          ...
          <p className="earn-subtitle">Tu ganancia después de comisión ({commissionPct}%)</p>
        </div>
      )}
      ...
      <div className="earn-kpis">
        ...
        <CountTile label="Suscripciones activas" value={totalActive} icon={<Users size={18} strokeWidth={1.5} />} />
        <CountTile label="Órdenes" value={totalOrders} icon={<ShoppingBag size={18} strokeWidth={1.5} />} />
      </div>

      {!programId && byMonth.length > 0 && (
        <Card className="earn-chart-card">
          <p className="earn-label">Ingresos por mes</p>
          <div className="earn-chart">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={byMonth} margin={{ top: 8, right: 12, left: -6, bottom: 0 }}>
                <defs>
                  {chartCurrencies.map((c) => (
                    <linearGradient key={c} id={`earnGrad-${c}`} x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={CUR_COLOR[c] || 'rgba(255,255,255,0.5)'} stopOpacity={0.18} />
                      <stop offset="95%" stopColor={CUR_COLOR[c] || 'rgba(255,255,255,0.5)'} stopOpacity={0} />
                    </linearGradient>
                  ))}
                </defs>
                <CartesianGrid strokeDasharray="0" stroke="rgba(255,255,255,0.04)" vertical={false} />
                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: 'rgba(255,255,255,0.3)', fontSize: 10 }} tickFormatter={fmtMonth} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10 }} width={48} />
                <Tooltip content={<EarnTooltip />} />
                {chartCurrencies.map((c) => (
                  <Area key={c} type="monotone" dataKey={c} name={c} stroke={CUR_COLOR[c] || 'rgba(255,255,255,0.8)'} strokeWidth={1.5} fill={`url(#earnGrad-${c})`} dot={false} activeDot={{ r: 3, strokeWidth: 0 }} />
                ))}
              </AreaChart>
            </ResponsiveContainer>
          </div>
      ...
      {!programId && programs.length > 0 && (
        <div className="earn-section">
          ...
          <div className="earn-programs">
            {programs.map((p) => <ProgramRow key={p.courseId} program={p} />)}
          </div>
        </div>
      )}
      ...
      <div className="earn-section">
        ...
        {orders.length === 0 ? (
          <div className="earn-empty earn-empty--wide">
            <Receipt size={20} strokeWidth={1.5} />
            <span>Sin órdenes todavía</span>
          </div>
        ) : (
          <div className="earn-orders">
