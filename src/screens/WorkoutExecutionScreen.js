...
import objectivesInfoService from '../services/objectivesInfoService';
...
import { trackScreenView, trackWorkoutStarted, trackWorkoutCompleted } from '../services/monitoringService';
...
const WorkoutExecutionScreen = ({ navigation, route }) => {
  ...
  const addExerciseModalVideoPlayer = useVideoPlayer('', (player) => {
    player.loop = true;
  ...
  const swapModalVideoPlayer = useVideoPlayer('', (player) => {
    player.loop = true;
    player.muted = isMuted;
  });
  ...
  const intensityVideoPlayer = useVideoPlayer('', (player) => {
    if (player) {
  ...
  useEffect(() => {
    // Track screen view and workout start
  ...
  useEffect(() => {
    ...
    // Preload next video for better UX
    preloadNextVideo();
  ...
  useEffect(() => {
    ...
    if (videoPlayer && canStartVideo) {
      ...
      logger.log('🎬 Video not ready:', { 
        reason: !videoPlayer ? 'no player' : !canStartVideo ? 'tutorial blocking' : 'unknown'
      });
    }
  ...
  const handleObjectiveCardPress = (objective) => {
  ...
  const loadPreviousSessionData = async () => {
    try {
      ...
      const exerciseHistoryPromises = workout.exercises.map(async (exercise) => {
        try {
          ...
          const exerciseHistoryData = await exerciseHistoryService.getExerciseHistory(user.uid, exerciseKey);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Three-Dot Menu Button - Left Position */}
      <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  content: {
    ...
    paddingBottom: Math.max(80, screenHeight * 0.1),
      ...
      exerciseTitleCard: {
        ...
        shadowRadius: 2,
        elevation: 2,
        height: Math.max(430, screenHeight * 0.53), // Responsive height: 60% of screen height, min 430px
  ...
  leftMenuButton: {
    position: 'absolute',
    top: Math.max(50, screenHeight * 0.075), // Below status bar
  ...
  dot: {
    width: Math.max(4, screenWidth * 0.01),
    height: Math.max(4, screenWidth * 0.01),
  ...
  endWorkoutButton: {
    ...
    justifyContent: 'center',
    alignSelf: 'center',
    shadowColor: 'rgba(255, 68, 68, 0.72)',
    shadowOffset: {
      width: 0,
      height: 0,
    },
